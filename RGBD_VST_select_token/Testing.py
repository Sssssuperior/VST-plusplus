...
def save_loss(save_dir, dataset, flops):
    fh = open(save_dir, 'a') 
    fh.write('dataset' + dataset + 'flops' + str(flops) + '\n')
    fh.write('\n')
    fh.close()
    ...
    for test_dir_img in test_paths:
        ...
        for i, data_batch in enumerate(test_loader):
            ...
            depth_14, depth_28, depth_56 = Variable(depth_14.cuda()), Variable(depth_28.cuda()), Variable(
                depth_56.cuda())
            mask_8 = torch.ones(787).cuda()
            mask_4 = torch.ones(3139).cuda()
            starts = time.time()
            outputs_saliency, outputs_contour,outputs_saliency_s, outputs_contour_s = net(images, depths, depth_14, depth_28, depth_56, mask_8, mask_4)
            ends = time.time()
            time_use = ends - starts
            time_list.append(time_use)
            #flops = FlopCountAnalysis(net, (images, depths, depth_14, depth_28, depth_56, mask_8, mask_4))
            #m = flops.total()
            #Flops += int(m)
        ...
        save_loss(save_timedir, test_dir_img.split('/')[0], Flops//len(test_loader.dataset))
