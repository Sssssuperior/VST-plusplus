...
class ImageData(data.Dataset):
    def __init__(self, dataset_list, data_root, transform, depth_transform, mode, label_14_transform, label_28_transform, label_56_transform, img_size=None, scale_size=None, t_transform=None, label_112_transform=None):
    ...
    def __getitem__(self, item):
        ...
        depth_p = Image.open(self.depth_path[item]).convert('L')
        ...
        if self.mode == 'train':
            ...
            new_depth_p = trans.Scale((random_size, random_size))(depth_p)
            ...
            if w != self.img_size and h != self.img_size:
                ...
                new_depth_p = new_depth_p.crop((x1, y1, x1 + self.img_size, y1 + self.img_size))
            ...
            if random.random() < 0.5:
                ...
                new_depth_p = new_depth_p.transpose(Image.FLIP_LEFT_RIGHT)

            new_img = self.transform(new_img)
            new_depth = self.depth_transform(new_depth)
            #new_depth_p = self.depth_transform_p(new_depth_p)
            #print(new_depth_p.shape)
            ...
            depth_14 = self.label_14_transform(new_depth_p)
            depth_28 = self.label_28_transform(new_depth_p)
            depth_56 = self.label_56_transform(new_depth_p)
            
            return new_img, new_depth, label_224, label_14, label_28, label_56, label_112, \
                   contour_224, contour_14, contour_28, contour_56, contour_112, depth_14, depth_28, depth_56 

        else:

            image = self.transform(image)
            depth = self.depth_transform(depth)
            depth_14 = self.label_14_transform(depth_p)
            depth_28 = self.label_28_transform(depth_p)
            depth_56 = self.label_56_transform(depth_p)

            return image, depth, image_w, image_h, self.image_path[item], depth_14, depth_28, depth_56
...
def get_loader(dataset_list, data_root, img_size, mode='train'):
    ...
    if mode == 'train':
        dataset = ImageData(dataset_list, data_root, transform, depth_transform,  mode, label_14_transform, label_28_transform, label_56_transform, img_size, scale_size, t_transform, label_112_transform)
    else:
        dataset = ImageData(dataset_list, data_root, transform, depth_transform, mode, label_14_transform, label_28_transform, label_56_transform)
