...
class decoderAttention(nn.Module):
    ...
    def forward(self, x, pos, mask):
        if mask != None:
            ...
            mask = mask.masked_fill(mask < 0.5, float(-100.0)).masked_fill(mask >= 0.5, float(0.0)).unsqueeze(1).repeat(1, self.num_heads,1,1)
            attn = (q @ k.transpose(-2, -1)) * self.scale  # B,num_heads,786,787
            # attn B,6,784(786),784(787)+ mask B,6,1,784
            attn[:,:,:,1:N-1]= attn[:,:,:,1:N-1] + mask
            attn = attn.softmax(dim=-1)
        else:
            B, N, C = x.shape
