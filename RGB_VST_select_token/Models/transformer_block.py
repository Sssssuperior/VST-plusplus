...
class decoderAttention(nn.Module):
    ...
    def forward(self, x, pos, mask):
        if mask != None:
            ...
            q = self.q(q_x).reshape(B, N, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3) #786,384
            k = self.k(k_x).reshape(B, N+1, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3) #787,384
            v = self.v(kv_x).reshape(B, N+1, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3)
           
            mask = mask.unsqueeze(1).repeat(1, self.num_heads, 1, 1)
            mask_sal = torch.ones(B, self.num_heads, 1, 1).cuda()
            mask_con = torch.ones(B, self.num_heads, 1, 1).cuda()
            mask_back = torch.ones(B, self.num_heads, 1, 1).cuda()
            mask = torch.cat((mask_sal, mask), dim=3)
            mask = torch.cat((mask, mask_con), dim=3)
            mask = torch.cat((mask, mask_back), dim=3)
            # x B,787,384 pos B,787,384
            
            mask_k = mask.ge(0.5).permute(0, 1, 3, 2)  # 1,6,787,1
            k = torch.masked_select(k, mask_k)
            k = k.reshape(B, self.num_heads, -1, C // self.num_heads)  # 1,6,N,64
            v = torch.masked_select(v, mask_k)
...
class Block_decoder(nn.Module):
    ...
    def forward(self, x, pos, mask=None):
        x = x + self.drop_path(self.attn(self.norm1(x),pos,mask))
        x = x + self.drop_path(self.mlp(self.norm2(x)))
