...
def main(local_rank, num_gpus, args):
    ...
    for epoch in range(args.epochs):
        ...
        for i, data_batch in enumerate(train_loader):
            ...
            print(
                'whole_iter_num: {0} --- {1:.4f} --- total_loss: {2:.6f}  --- loss2: {3:.6f} --- loss2: {4:.6f}'.format(
                    (whole_iter_num + 1),
                    (i + 1) * args.batch_size / N_train, total_loss.item(), img_total_loss_i.item(),img_total_loss_s_i.item()))
